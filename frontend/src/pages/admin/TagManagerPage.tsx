...
export function TagManagerPage() {
  ...
  const deleteMutation = useMutation({
    mutationFn: (id: string) => adminDelete('tags', id),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['admin-tags'] });
      queryClient.invalidateQueries({ queryKey: ['tags'] });
      setDeleteError('');
            ...
            {data?.map(tag => (
              <li key={tag.id} className="admin-item-row">
                ...
                <span className="admin-item-names">
                  {tag.translations.map(t => t.name).filter(Boolean).join(' / ') || '—'}
                </span>
                <button
                  type="button"
          ...
          <form onSubmit={handleCreate} className="admin-form">
            ...
            {languages?.map(lang => (
              <div key={lang.code} className="form-row">
                <label htmlFor={`tag-name-${lang.code}`}>Name ({lang.name})</label>
                <input
                  id={`tag-name-${lang.code}`}
